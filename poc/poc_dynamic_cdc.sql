...
def merge_cdc_handler(
    ...
    incremental_source_df = source_df.filter(col("IS_PROCESSED")==False)
    ...
    deduped_source_df = source_df.select(
        "*", 
    ...
    # staging table is_processed = false 를 true로 변경
...
--처리 플래그 생성
ALTER TABLE poc.poc.staging_table ADD COLUMN IS_PROCESSED BOOLEAN DEFAULT FALSE;
...
select * from staging_table where is_processed = true;
